...
async function completeProjects() {
  return new Promise((resolve, reject) => {
    ...
    db.serialize(async () => {
      try {
        console.log('🌱 Completing project data creation...');

        // Get existing locations
        const existingLocations = await new Promise((resolve, reject) => {
        ...
        for (const location of allLocations) {
          if (!locationMap[location.name]) {
            ...
            console.log(`✅ Location already exists: ${location.name} (ID: ${locationMap[location.name]})`);
          }
        }
...
completeProjects().catch(console.error);
