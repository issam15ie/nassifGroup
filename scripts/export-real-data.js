// Script to export REAL data from Strapi database to static JSON format
...
async function exportRealData() {
  console.log('📤 Exporting REAL data from Strapi database...');
  ...
  try {
    ...
    console.log(`📊 Found ${apartments.length} apartments, ${projects.length} projects, ${propertyTypes.length} property types`);
    ...
    // Convert property types to static format
    ...
    const summary = {
      ...
      apartments: staticApartments.length,
      projects: staticProjects.length,
      propertyTypes: staticPropertyTypes.length,
      featuredApartments: staticApartments.filter(apt => apt.isFeatured).length,
      availableApartments: staticApartments.filter(apt => apt.status === 'available').length,
      soldApartments: staticApartments.filter(apt => apt.status === 'sold').length
    };
    
    const summaryPath = path.join(staticDir, 'export-summary.json');
    fs.writeFileSync(summaryPath, JSON.stringify(summary, null, 2));
    console.log(`✅ Export summary saved to ${summaryPath}`);
    
    console.log('\n🎉 REAL DATA EXPORT COMPLETED!');
    console.log('📁 Static data files created in: nassif-group-static/data/');
    console.log('🌐 Your static website now has ALL your real database data!');
    
    // Show summary
    console.log('\n📊 EXPORT SUMMARY:');
    console.log(`   🏠 Apartments: ${summary.apartments} (${summary.availableApartments} available, ${summary.soldApartments} sold)`);
    console.log(`   🏢 Projects: ${summary.projects}`);
    console.log(`   🏘️ Property Types: ${summary.propertyTypes}`);
    console.log(`   ⭐ Featured: ${summary.featuredApartments}`);
