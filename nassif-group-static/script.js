// Nassif Group Website JavaScript - Static Version
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    dataManager = new WebsiteDataManager();
    
    // Wait for data to load
    setTimeout(() => {
        // Initialize static data integration
        initializeStaticIntegration();
    }, 100);
...
async function initializeStaticIntegration() {
    try {
        // Wait for static data to load
        await new Promise(resolve => {
            const checkData = () => {
                if (strapiAPI.projects.length > 0 && strapiAPI.apartments.length > 0) {
                    resolve();
                } else {
        ...
        console.log('✅ Static data integration initialized successfully');
        ...
        console.error('❌ Static integration failed:', error);
    }
}
...
async function loadApartmentsPage() {
    try {
        // Load projects and property types from data manager
...
async function loadProjectsAndPropertyTypes() {
    try {
        ...
        const projectsResponse = await dataManager.getProjects();
        const propertyTypesResponse = await dataManager.getPropertyTypes();
        
        if (projectsResponse.data && propertyTypesResponse.data) {
            // Process projects data
            PROJECTS_DATA = {};
            projectsResponse.data.forEach(project => {
                // Handle both Strapi format (with attributes) and static format (direct properties)
                const projectAttrs = project.attributes || project;
                
                const projectData = {
                    id: project.id,
                    name: projectAttrs.name,
                    status: projectAttrs.status,
                    description: projectAttrs.description,
                    propertyTypeInfo: projectAttrs.propertyTypeInfo || {},
                    propertyTypes: {},
                    propertyTypesOrder: [] // Store the order of property types
                };
                
                // Process property types - handle both Strapi format and static format
                if (projectAttrs.propertyTypes) {
                    // Handle static format (array of strings) or Strapi format (with data array)
                    const propertyTypesArray = Array.isArray(projectAttrs.propertyTypes) 
                        ? projectAttrs.propertyTypes 
                        : (projectAttrs.propertyTypes.data || []);
                    
                    propertyTypesArray.forEach(pt => {
                        // Handle both string format and object format
                        const propertyTypeName = typeof pt === 'string' ? pt : (pt.attributes?.name || pt.name);
                ...
                PROJECTS_DATA[projectAttrs.name] = projectData;
            });
            ...
            propertyTypesResponse.data.forEach(pt => {
                // Handle both Strapi format (with attributes) and static format (direct properties)
                const propertyTypeAttrs = pt.attributes || pt;
                
                PROPERTY_TYPES_DATA[propertyTypeAttrs.name] = {
                    id: pt.id,
                    name: propertyTypeAttrs.name,
                    displayName: propertyTypeAttrs.displayName,
                    range: `${propertyTypeAttrs.minSize || 0}-${propertyTypeAttrs.maxSize || 0}${propertyTypeAttrs.sizeUnit || 'm²'}`,
                    image: propertyTypeAttrs.image?.data?.attributes?.url || propertyTypeAttrs.image || getDefaultImage(propertyTypeAttrs.name),
                    description: propertyTypeAttrs.description,
                    features: propertyTypeAttrs.features || []
                };
            });
