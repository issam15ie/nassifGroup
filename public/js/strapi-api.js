...
class StrapiAPI {
    ...
    async getFeaturedApartments(limit = 3) {
        // Use regular apartments endpoint with featured filter
        return await this.apiCall(`/apartments?filters[isFeatured][$eq]=true&pagination[limit]=${limit}&populate=mainImage,project`);
    }
    ...
    // Get all projects
    async getProjects() {
        return await this.apiCall('/projects?populate[propertyTypes]=*&populate[mainImage]=*&populate[images]=*&populate[units][populate][images]=*&populate[propertyTypeOverrides]=*');
    }

    // Get project by ID or slug
    async getProject(id) {
        return await this.apiCall(`/projects/${id}?populate=propertyTypes,mainImage,images,units.images,propertyTypeOverrides`);
    }
